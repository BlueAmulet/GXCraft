...
inline void drawLeavesBlock(int xPos, int yPos, int zPos) {
	...
	bool drawBack  = renderAllFaces || zPos >= worldZ-1 || testLeavesFace(theWorld[yPos][xPos][zPos + 1]);
	bool drawFront = renderAllFaces || zPos <= 0        || testLeavesFace(theWorld[yPos][xPos][zPos - 1]);
	bool drawRight = renderAllFaces || xPos >= worldX-1 || testLeavesFace(theWorld[yPos][xPos + 1][zPos]);
	bool drawLeft  = renderAllFaces || xPos <= 0        || testLeavesFace(theWorld[yPos][xPos - 1][zPos]);
	bool drawTop   = renderAllFaces || yPos >= worldY-1 || testLeavesFace(theWorld[yPos + 1][xPos][zPos]);
	bool drawBott  = renderAllFaces || (yPos > 0        && testLeavesFace(theWorld[yPos - 1][xPos][zPos]));
