...
static void renderCube(s16 xPos, s16 yPos, s16 zPos, bool drawBack, bool drawFront, bool drawRight, bool drawLeft, bool drawTop, bool drawBott, blockTexture *texTop, blockTexture *texSide, blockTexture *texBott) {
	if (!drawBack && !drawFront && !drawRight && !drawLeft && !drawTop && !drawBott)
		return;
	...
	if (drawBack) {
		...
		dlist->add(    xPos, 1 + yPos, 1 + zPos, c, u0, v0);
		dlist->add(1 + xPos, 1 + yPos, 1 + zPos, c, u1, v0);
		dlist->add(1 + xPos,     yPos, 1 + zPos, c, u1, v1);
		dlist->add(    xPos,     yPos, 1 + zPos, c, u0, v1);
	}

	if (drawFront) {
		unsigned short c = (zPos > 0 && theWorld->lighting[xPos][zPos - 1] > yPos) ? 0x888F : 0xCCCF;

		dlist->add(1 + xPos, 1 + yPos, zPos, c, u0, v0);
		dlist->add(    xPos, 1 + yPos, zPos, c, u1, v0);
		dlist->add(    xPos,     yPos, zPos, c, u1, v1);
		dlist->add(1 + xPos,     yPos, zPos, c, u0, v1);
	}

	if (drawRight) {
		unsigned short c = (xPos < worldX - 1 && theWorld->lighting[xPos + 1][zPos] > yPos) ? 0x777F : 0x999F;

		dlist->add(1 + xPos, 1 + yPos, 1 + zPos, c, u0, v0);
		dlist->add(1 + xPos, 1 + yPos,     zPos, c, u1, v0);
		dlist->add(1 + xPos,     yPos,     zPos, c, u1, v1);
		dlist->add(1 + xPos,     yPos, 1 + zPos, c, u0, v1);
	}

	if (drawLeft) {
		unsigned short c = (xPos > 0 && theWorld->lighting[xPos - 1][zPos] > yPos) ? 0x777F : 0x999F;

		dlist->add(xPos, 1 + yPos,     zPos, c, u0, v0);
		dlist->add(xPos, 1 + yPos, 1 + zPos, c, u1, v0);
		dlist->add(xPos,     yPos, 1 + zPos, c, u1, v1);
		dlist->add(xPos,     yPos,     zPos, c, u0, v1);
	}
	...
	if (drawTop) {
		...
		dlist->add(    xPos, 1 + yPos,     zPos, c, u0, v0);
		dlist->add(1 + xPos, 1 + yPos,     zPos, c, u1, v0);
		dlist->add(1 + xPos, 1 + yPos, 1 + zPos, c, u1, v1);
		dlist->add(    xPos, 1 + yPos, 1 + zPos, c, u0, v1);
	}
	...
	if (drawBott) {
		...
		dlist->add(1 + xPos, yPos,     zPos, 0x555F, u0, v0);
		dlist->add(    xPos, yPos,     zPos, 0x555F, u1, v0);
		dlist->add(    xPos, yPos, 1 + zPos, 0x555F, u1, v1);
		dlist->add(1 + xPos, yPos, 1 + zPos, 0x555F, u0, v1);
	}
	// clang-format on
}
...
namespace Render {
	...
	void drawBlock(s16 xPos, s16 yPos, s16 zPos, blockTexture *tex) {
		drawMultiTexBlock(xPos, yPos, zPos, tex, tex, tex);
	}

	void drawBlockCrossed(s16 xPos, s16 yPos, s16 zPos, blockTexture *tex) {
		unsigned short c = (theWorld->lighting[xPos][zPos] > yPos) ? 0x999F : 0xFFFF;
	...
	void drawMultiTexBlock(s16 xPos, s16 yPos, s16 zPos, blockTexture *texTop, blockTexture *texSide, blockTexture *texBott) {
	...
	void drawLiquidBlock(s16 xPos, s16 yPos, s16 zPos, blockTexture *tex) {
		uint8_t blockID = theWorld->theWorld[yPos][xPos][zPos];
		// clang-format off
		bool drawBack  = zPos >= worldZ - 1 || testFace(blockID, theWorld->theWorld[yPos][xPos][zPos + 1]);
		bool drawFront = zPos <= 0          || testFace(blockID, theWorld->theWorld[yPos][xPos][zPos - 1]);
	...
	void drawSelectionBlock(f32 xPos, f32 yPos, f32 zPos) {
		GX_Begin(GX_LINES, GX_VTXFMT0, 24);

		GX_Position3f32(xPos - 0.01f, yPos + 1.01f, zPos + 1.01f);
		GX_Color1u32(0x000000CC);
		GX_TexCoord2f32(0, 0);
		GX_Position3f32(xPos + 1.01f, yPos + 1.01f, zPos + 1.01f);
		GX_Color1u32(0x000000CC);
		GX_TexCoord2f32(0, 0);
		GX_Position3f32(xPos + 1.01f, yPos + 1.01f, zPos + 1.01f);
		GX_Color1u32(0x000000CC);
		GX_TexCoord2f32(0, 0);
		GX_Position3f32(xPos + 1.01f, yPos - 0.01f, zPos + 1.01f);
		GX_Color1u32(0x000000CC);
		GX_TexCoord2f32(0, 0);
		...
		GX_Color1u32(0x000000CC);
		GX_TexCoord2f32(0, 0);
		GX_Position3f32(xPos - 0.01f, yPos - 0.01f, zPos + 1.01f);
		GX_Color1u32(0x000000CC);
		GX_TexCoord2f32(0, 0);
		GX_Position3f32(xPos - 0.01f, yPos - 0.01f, zPos + 1.01f);
		GX_Color1u32(0x000000CC);
		...
		GX_Position3f32(xPos - 0.01f, yPos + 1.01f, zPos - 0.01f);
		GX_Color1u32(0x000000CC);
		GX_TexCoord2f32(0, 0);
		GX_Position3f32(xPos + 1.01f, yPos + 1.01f, zPos - 0.01f);
		GX_Color1u32(0x000000CC);
		GX_TexCoord2f32(0, 0);
		GX_Position3f32(xPos + 1.01f, yPos + 1.01f, zPos - 0.01f);
		GX_Color1u32(0x000000CC);
		GX_TexCoord2f32(0, 0);
		GX_Position3f32(xPos + 1.01f, yPos - 0.01f, zPos - 0.01f);
		GX_Color1u32(0x000000CC);
		GX_TexCoord2f32(0, 0);
		...
		GX_Color1u32(0x000000CC);
		GX_TexCoord2f32(0, 0);
		GX_Position3f32(xPos - 0.01f, yPos - 0.01f, zPos - 0.01f);
		GX_Color1u32(0x000000CC);
		GX_TexCoord2f32(0, 0);
		GX_Position3f32(xPos - 0.01f, yPos - 0.01f, zPos - 0.01f);
		GX_Color1u32(0x000000CC);
