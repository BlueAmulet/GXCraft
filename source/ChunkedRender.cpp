...
static size_t renderorder[nRenderChunks];

static int getchunkfromchunkpos(short x, short z) {
...
static inline void renderNewChunk(short cx, short cz) {
...
namespace Chunked {
	void init() {
		...
		for (size_t i = 0; i < nRenderChunks; i++) {
	...
	void markchunkforupdate(short x, short z) {
...
#define testChunk(xval, yval, zval) \
	...
	if (polygon.x >= 0) \
		allL = false; \
	if (polygon.x <= rmode->fbWidth) \
		allR = false; \
	if (polygon.y >= 0) \
		allU = false; \
	if (polygon.y <= rmode->efbHeight) \
		allD = false; \
	if (polygon.z > 0) \
		vZ = true;
...
#undef testChunk
		...
		qsort(renderorder, nrendered, sizeof(size_t), chunk_cmp);
		for (size_t i = 0; i < nrendered; i++) {
		...
		GRRLIB_ObjectViewTrans(-thePlayer.posX, -thePlayer.posY - 1.625f - (1.5f / 16.0f), -thePlayer.posZ);
	...
	void getfifousage(size_t *usage, size_t *total) {
		size_t _usage = 0;
		size_t _total = 0;
		...
		for (size_t i = 0; i < nRenderChunks; i++) {
			RenderChunk *rc = renderchunks[i];
			if (rc->active && rc->list) {
				_usage += rc->list->index - 1u;
				_usage += rc->blendlist->index - 1u;
				_total += rc->list->size;
